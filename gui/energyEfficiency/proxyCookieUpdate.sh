...
# Port
printf "Getting port: "
port=`grep -Poh '"port":\s*?"\K.*(?=",?)' $proxyConfig`
if [ -n "$port" ]
then
  echo $port
else
...
curl -k --request POST "$protocol://$host:$port/login" -d IDToken1="$username" -d IDToken2="$password" -c "$cookiejar" && echo "OK"
...
#cdt2 serve --proxy-config proxyConfig.json
